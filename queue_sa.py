...
# Description: This code defines the class 'Queue' that represents a queue data structure using a static array as
# its underlying data storage
...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        ...
        if self._current_size == self._sa.length():
            # Resize the underlying StaticArray if it's full
            self._double_queue()

        # Increment the back index and wrap around if necessary
        self._back = self._increment(self._back)
        self._sa[self._back] = value
        # Keep track of elements in the queue by incrementing the self._current_size variable by 1
    ...
    def dequeue(self) -> object:
        ...
        # Move the front index to the next position
    ...
    def front(self) -> object:
        ...
        # If the queue is not empty, return the value of the front element of the queue
    ...
    def _double_queue(self) -> None:
        """
        This is a helper method to assist with resizing
        """
        # Create a new static array with double the size
        ...
        # Update the front and back indices
        self._front = 0
        self._back = self._current_size - 1

        # Replace the old static array with the new one
        self._sa = new_sa
